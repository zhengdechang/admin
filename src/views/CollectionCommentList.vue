...
<script>
    export default {
        ...
        methods:{
            async fetch(){
                const res = await this.$http.get('/comment/album/')
                console.log(res);
                this.items = res.data
            },
            toEdit(scope_id){
                this.$router.push({path:`/comment/edit/`,query:{type:'collection',id:scope_id,}});
            },
            async remove(row){
                    this.$confirm(`是否确认删除"${row.user_id.username}"的这条评论评论` , '提示', {
                        ...
                        await this.$http.delete(`/comment/album/${row._id}`)
                        this.$message({
                            type: 'success',
                            message: '删除成功!'
                        });
